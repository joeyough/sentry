...
- **Lab** — Natural-language feature requests. Generates specs with red tests and a live sandbox preview. The key-man-risk killer.

Auto light/dark theme. Responsive — icon-only tabs on mobile.
...
- **Digest** — Weekly summary for the board with confirmed incident count.

Auto light/dark theme. Fraunces + Inter typography. Phone-framed preview.
...
| Lab sandbox | Original | Natural language to spec + tests + live preview |
...
Proprietary. VDA Labs internal use only.| FP rate | Is noise improving? | Tracked from triage decisions |
| Renewal signal | Does sales mention alerting? | Yes/no from sales team |
